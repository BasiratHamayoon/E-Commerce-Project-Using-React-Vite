...
import { AiFillHeart, AiOutlineHeart } from "react-icons/ai"; // Import AiOutlineHeart for the unfilled heart
...
const FavoriteProducts = () => {
  ...
  const handleFavorite = (data) => {
    dispatch(toggleFavorite(data)); // This will add or remove the product from the favorites list
  };
                ...
                <span className="flex justify-center sm:justify-start text-[#101750] gap-4 py-2 cursor-pointer">
                  ...
                  {isFavorite(item) ? (
                    <AiFillHeart
                      ...
                      onClick={() => handleFavorite(item)} // This will remove the product from favorites when clicked
                    />
                  ) : (
                    <AiOutlineHeart
                      className="text-red-600"
                      onClick={() => handleFavorite(item)} // This will add the product to favorites when clicked
                    />
                  )}
                  <LiaSearchPlusSolid />
                </span>
